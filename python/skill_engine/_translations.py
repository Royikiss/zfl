"""Shared translations data and loading for the skill engine."""

import json
import os

from ._display import IS_ZH
from ._store import TRANSLATIONS_FILE, atomic_save_json
...
def load_user_translations():
    """Load Chinese translations from TRANSLATIONS_FILE, merging defaults for missing entries."""
    if not os.path.exists(TRANSLATIONS_FILE):
        atomic_save_json(TRANSLATIONS_FILE, DEFAULT_TRANSLATIONS)
        ...
        with open(TRANSLATIONS_FILE, "r", encoding="utf-8") as f:
            ...
            for k, v in DEFAULT_TRANSLATIONS.items():
                if k not in data:
                    ...
                    updated = True
            if updated:
                atomic_save_json(TRANSLATIONS_FILE, data)
            return data
    except Exception:
        return DEFAULT_TRANSLATIONS
