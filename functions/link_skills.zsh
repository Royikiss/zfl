...
link_skills() {
    ...
    if (( ${#skills_to_link[@]} == 0 )); then
        if (( $+commands[fzf] )); then
            local prompt_msg
            if [[ "$lang" == zh* ]]; then
                prompt_msg="选择要链接的 Skill/分组 (空格多选，ctrl-g:创建/更新组，ctrl-d:删除组，Enter确认): "
            else
                prompt_msg="Select skills/groups to link (Space to multi-select, ctrl-g:create/update group, ctrl-d:delete group, Enter to confirm): "
            fi
